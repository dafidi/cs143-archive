...
/*
    let_formal_list : ',' OBJECTID ':' TYPEID
    { $$ = let($2, $4, no_expr(), no_expr()); }
    | ',' OBJECTID ':' TYPEID ASSIGN expression
    { $$ = let($2, $4, $6, no_expr()); }
    | ',' OBJECTID ':' TYPEID let_formal_list
    { $$ = let($2, $4, no_expr(), no_expr()); }
    | ',' OBJECTID ':' TYPEID ASSIGN expression let_formal_list
    { $$ = let($2, $4, $6, no_expr()); }
    ;
    ...
    | expression
    { $$ = $1; }
    | expression ',' OBJECTID ':' TYPEID
    { $$ = $1; }
    | expression ',' OBJECTID ':' TYPEID ASSIGN expression
    { $$ = $1; }
    | expression let_init_list
    { $$ = $1; }
    | expression ',' OBJECTID ':' TYPEID let_init
    { $$ = $1; }
    | expression ',' OBJECTID ':' TYPEID ASSIGN expression let_init
    { $$ = $1; }
//was inside expresion
    | LET OBJECTID ':'  TYPEID IN expression
    { $$ = let($2, $4, no_expr(), $6); }
    | LET OBJECTID ':'  TYPEID ASSIGN expression IN expression
    { $$ = let($2, $4, $6, $8); }
    | LET OBJECTID ':'  TYPEID let_formal_list IN expression
    { $$ = let($2, $4, no_expr(), $7); }
    | LET OBJECTID ':'  TYPEID ASSIGN expression let_formal_list IN expression
    { $$ = let($2, $4, $6, $9); }
    | LET OBJECTID ':'  TYPEID ASSIGN let_formal_list IN expression
    { $$ = let($2, $4, $6, $8); }
    */
